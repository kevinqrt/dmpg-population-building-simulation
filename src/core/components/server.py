...
from dmpg_logs.logging_utils.stats_logger import log_server_statistics
...
from src.core.components.oven import Oven
...
class Server(ProcessingComponent):
    """
    Simulates a server within a discrete event simulation environment.

    This server processes entities based on specified parameters, such as
    ...
    servers = Model().get_component(ComponentType.SERVERS)
    """Manages all existing server instances."""
    ...
    def __init__(self, env: simpy.Environment, name: str,
        ...
        # Oven setup
        self.oven = Oven() if oven else None
    ...
    def _process_entity(self, worker):
        ...
        logging.root.level <= logging.TRACE and logging.trace(
            ENTITY_PROCESSING_LOG_ENTRY.format(f"[Server] {self.name} begins processing {entity.name} , worker={worker_info}",
        ...
        logging.root.level <= logging.TRACE and logging.trace(
            ENTITY_PROCESSING_LOG_ENTRY.format(f"[ServerStats] Server={self.name}, Status=busy, Entity={entity.name}, "
                                               f"Queue={self.queue_length}, Total={self.total_entities_processed_pivot_table}",
                                               DateTime.get(self.env.now)
                                               )
        )

        logging.root.level <= logging.TRACE and logging.trace(
            ENTITY_PROCESSING_LOG_ENTRY.format(f"[Server] {self.name} starts processing {entity.name}",
        ...
        if self.time_between_machine_breakdowns:
            ...
            yield from self._process_with_oven(processing_time)
        else:
        ...
        logging.root.level <= logging.TRACE and logging.trace(
            ENTITY_PROCESSING_LOG_ENTRY.format(
                f"[Server] {self.name} finished processing {entity.name}, time {round_value(processing_time)}",
                DateTime.get(self.env.now)))
        ...
        if self.env.now >= gi.DURATION_WARM_UP:
            ...
            logging.root.level <= logging.TRACE and logging.trace(
                ENTITY_PROCESSING_LOG_ENTRY.format(
                    f"[ServerStats] Server={self.name}, Status=idle, Entity=-, Queue={self.queue_length}, Total={self.total_entities_processed_pivot_table}",
                    DateTime.get(self.env.now)
                )
            )
        ...
        #log_server_statistics(self, effective_time=effective_time, total_time=self.env.now)
    ...
    # Reset of the Server class remains unchanged
    def _process_with_oven(self, processing_time: Union[int, float]) -> None:
        """
        Simulates entity processing with oven heating/cooling behavior.
        """
        # Simulate oven heating and processing
        self.oven.calculate_oven(self.env.now)
        self.oven.update_oven(door_status=True, heating_status=True)
        yield self.env.timeout(2)
        self.oven.update_oven(door_status=False, heating_status=True)
        yield self.env.timeout(self.oven.calculate_time_to_required_temperature())
        yield self.env.timeout(processing_time)
        self.oven.update_oven(door_status=False, heating_status=False)
